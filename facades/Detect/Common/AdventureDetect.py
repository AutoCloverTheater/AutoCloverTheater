...
class AdventureDetect:
    ...
    def isInMainUi(self):
        ...
        pot, ok  = imgSearchArea(GetSnapShot().img, mainWindow, [773,622,65,86])
        if ok:
            pot = pot.pop()
    ...
    def isInAdventureList(self):
        ...
        mainWindow = MyImread(path)
        pot, ok  = imgSearchArea(GetSnapShot().img, mainWindow, [96,23,93,22])
        if ok:
            pot = pot.pop()
    ...
    def hasWorldTreeButton(self):
        ...
        mainWindow = MyImread(path)
        pot, ok  = imgSearchArea(GetSnapShot().img, mainWindow, [0,336,1280,278])
        if ok:
            pot = pot.pop()
    ...
    def last(self):
        ...
        mainWindow = MyImread(path)
        pots, ok  = imgSearchArea(GetSnapShot().img, mainWindow, [0, 438, 1280, 278])
        if ok:
            pots = pots.pop()
